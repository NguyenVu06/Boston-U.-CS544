...
contUSdf$fitted_rent <- unclass(fitted(rent_mlrm2))
contUSdf$resid_rent <- unclass(resid(rent_mlrm2))

hist(resid_rent, xlab = "Residual Gross Rent")
skewness(resid_rent) #only slightly skewwed but resembled normal at skewness = 0.58

p + geom_point(aes(x=fitted_rent, y=resid_rent))+ geom_smooth(aes(x=fitted_rent, y=resid_rent), method = "lm", se = T) + xlab("Predicted Rent")+ ylab("Residual")
#############
#############
...
rent_mlrm <- lm(contUSdf$rent_median ~ contUSdf$Household_Income_median+
                  contUSdf$Pop_density+contUSdf$water_land_ratio+contUSdf$gNorth+contUSdf$gWest+contUSdf$gMid+
                  contUSdf$Owner_Costs_mortgage_median+contUSdf$Owner_Costs_median)
anova(rent_mlrm) #F ratio is significant
summary(rent_mlrm)
#R Square = 0.646 , Residual SE = 259.6

# water_land_ratio looks not to be significant. p>0.05. Reduce model
rent_mlrm2 <- lm(contUSdf$rent_median ~ Household_Income_median+
                   Pop_density+gNorth+gWest+gMid+
                   Owner_Costs_mortgage_median+Owner_Costs_median)
summary(rent_mlrm2)
suma <- summary(rent_mlrm2)
#get the slope estimates table for each predictor
suma$coefficients
